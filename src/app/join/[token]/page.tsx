...
function getRoleLabel(t: ReturnType<typeof useTranslations>, role: PeekOk['role']): string {
  return {
    admin: t('roleAdmin'),
    agent: t('roleAgent'),
    viewer: t('roleViewer'),
...
export default function JoinPage() {
  ...
  // ----- Peek failed -----
  if (!peek.ok) {
    const copy = getFailCopy(t, peek.reason);
    return (
      <Card className="w-full max-w-md border-border bg-card">
        ...
        <Card className="w-full max-w-md border-border bg-card">
          ...
          <CardContent className="flex flex-col gap-3">
            ...
            <p className="text-center text-xs text-muted-foreground">
              {t('acceptingDescPrefix')}{' '}
              <span className="text-muted-foreground">{peek.account_name}</span>. {t('acceptingDescSuffix')}
            </p>
          </CardContent>
        </Card>
