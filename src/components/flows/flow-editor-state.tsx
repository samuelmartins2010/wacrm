...
export function FlowEditorProvider({
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : t("saveFailedGeneric");
      toast.error(msg);
    } finally {
      setSaving(false);
    }
  ...
  const setStatus = useCallback(
    async (next: BuilderState["status"]) => {
      ...
      try {
        ...
        if (!res.ok) {
          const json = await res.json().catch(() => ({}));
          throw new Error(json.error ?? t("statusUpdateFailedWithStatus", { status: res.status }));
        }
        ...
        const msg = err instanceof Error ? err.message : t("statusUpdateFailedGeneric");
        toast.error(msg);
      } finally {
        setActivating(false);
      }
  ...
  const deleteFlow = useCallback(async () => {
    const yes = window.confirm(t("confirmDelete", { name: state.name }));
    if (!yes) return;
    try {
      const res = await fetch(`/api/flows/${initialFlow.id}`, {
        method: "DELETE",
      ...
      const msg = err instanceof Error ? err.message : t("deleteFailedGeneric");
      toast.error(msg);
    }
